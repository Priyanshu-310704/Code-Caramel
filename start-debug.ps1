...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$root\Algocode-Problem-Service'; Write-Host 'PROBLEM SERVICE (port 3003)' -ForegroundColor Cyan; node src/index.js > out.log 2>&1"

Start-Sleep -Seconds 1

# Start Socket Service (port 3001)
Write-Host "Starting Socket Service on port 3001..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$root\AlgoCode-Socket-Service'; Write-Host 'SOCKET SERVICE (port 3001)' -ForegroundColor Cyan; node src/server.js > out.log 2>&1"

Start-Sleep -Seconds 1

# Start Evaluator Service (port 3000)
Write-Host "Starting Evaluator Service on port 3000..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$root\Algocode-Evaluator-Service'; Write-Host 'EVALUATOR SERVICE (port 3000)' -ForegroundColor Cyan; node dist/index.js > out.log 2>&1"

Start-Sleep -Seconds 1

# Start Submission Service (port 3002)
Write-Host "Starting Submission Service on port 3002..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$root\AlgoCode-Submission-Service'; Write-Host 'SUBMISSION SERVICE (port 3002)' -ForegroundColor Cyan; node src/index.js > out.log 2>&1"
